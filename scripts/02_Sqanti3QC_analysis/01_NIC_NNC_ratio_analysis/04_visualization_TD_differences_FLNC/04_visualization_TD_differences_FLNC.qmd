...
ggplot(db_RBN_diff_filtered, 
  ...
  annotate("text", x = mean(range(db_RBN_diff_filtered$RBN_Difference[db_RBN_diff_filtered$RBN_Difference > 0])), 
           ...
           label = paste("T-cell+\n",
                         ...
                         percent_yes_amongDiff, "%)", 
                         sep = ""), 
           hjust = 0.5, size = 6, color = "#15918A") + 
  
  annotate("text", x = mean(range(db_RBN_diff_filtered$RBN_Difference[db_RBN_diff_filtered$RBN_Difference < 0])),
           y = annotation_y_diff, 
           label = paste("Control\n", count_no_filt, " genes \n(", 
                         percent_no_amongDiff, "%)", sep = ""), 
           hjust = 0.5, size = 6, color = "#F58A53") +
  labs(title = "Difference of Transcription divergency\nscore between samples", 
       y= "Gene counts", x = paste0("Difference of TD score  (", 
                                    total_genes_diff, " genes)" )) +
  theme(
...
```{r}
# Filtering to remove genes barely expressed (based on FDM FL counts)

db_RNB_Highdiff_filtered <- db_RBN_diff_filtered %>%
  filter(sample1_FSM > 5,
         sample2_FSM > 5)

# recount yes or no after this new filtering
count_values_Highdiff <- db_RNB_Highdiff_filtered %>%
  count(Greater_in_bc01) 

write_csv(db_RNB_Highdiff_filtered, "../../../../analysis/01_NIC_NNC_ratio_analysis/04_visualization_TD_differences_FLNC/TD_highTDscore_min5FLNC.csv")

print(count_values_Highdiff)
```

```{r}
write_csv(db_RBN_diff_filtered, "../../../../analysis/01_NIC_NNC_ratio_analysis/04_visualization_TD_differences_FLNC/TD_diffTDscore_FLNC.csv")
```
...
TD_per_chr_long <- TD_per_chr %>%
  select(chr, perc_TDhigherStress, perc_TDhigherControl,diff_perc) %>%
  pivot_longer(
    cols = c(perc_TDhigherStress, perc_TDhigherControl, diff_perc),
    names_to = "Condition",
  ...
  mutate(Condition = case_when(
    ...
    Condition == "diff_perc"  ~ "Difference of proportions"
  ))
...
ggplot(TD_per_chr_long, aes(x = chr, 
  ...
  geom_point(size = 3) +
  scale_color_manual(values = c("darkgrey","palevioletred3", "darkseagreen3")) +
  labs(
...
```{r}
# same graph with Conesa Lab colors
ggplot(TD_per_chr_long, aes(x = chr, 
                            y = Percentage, 
                            group = Condition, color = Condition)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  scale_color_manual(values = c("darkgrey","#F58A53", "#15918A")) +
  labs(
    title = "Genes (%) with increased Transcription divergency score",
    subtitle. = "by Chromosome and sample",
    x = "Chromosome",
  ...
  scale_x_discrete(limits = c(1:22, "X", "Y"))
```
