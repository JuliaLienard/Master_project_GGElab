...
SqantiQC was run and the classification output files generated are used here, after being merged with correct FLNC counts (from the flnc_counts.txt from IsoSeq Collapse). It focuses on the analysis of genes showing a difference in Transcription diversity (TD), also called RNA biological noise (RBN), that is calculated by looking at the proportion of NIC+NNC isoforms among NIC+NNC+ FSM isoforms.
...
# loading 00_Merge_SqQCclass_FLcounts/bc01_class_FLNC.txt
...
# loading 00_Merge_SqQCclass_FLcounts/bc02_class_FLNC.txt
...
normalizing the read counts (CPM calculation) is common and is calculated as follow:

> log2((FL_count/total_mapped_reads)\*10e6) +1)

The total_mapped_reads are the total FL_counts in the collapsed\_\<sample\>.flnc_count.txt or in \<sample\>\_class_FLNC.txt
...
# matches with the bash command results
