...
This part comes after preparing the data using 02b_TUSCOpancreas_ISM_Dataprep.qmd. Here we want to predict the ISM transcripts (%) using the TUSCO genes, to detect an increased ISM proportion for some genes that could correspond to TD.
...
```{r}
# loading Data_format_ISM_model.csv 
# (output from 02b_TUSCOpancreas_ISM_Dataprep)
...
ggplot(Data_ISM_predictors_TUSCO) +
  ...
  labs(title = "GC(%) of transcripts for TUSCO Selected Genes",
       y = "Density",
       x = "GC(%)",
       subtitle = "weighted by FL counts (pink)") 
```
...
It suggests here that if using the predictor of number of exons, we should use a transformed predictor.
...
GLM model is better than the linear since ISM is expressed in percentages and thus, the model will bound the ISM values between 0 an 100, compared to linear models than calculates any value. It is particularly suited when proportions are based on counts, which is the case here as FL counts were used to calculate ISM_perc.
